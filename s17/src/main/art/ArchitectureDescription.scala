...
@sig trait ArtSystem {
  // operations to execute after bridge/port setup
  def startup(): Unit

  // operations to execute after bridge finalize methods complete
  def shutdown(): Unit
}

@record class ArchitectureDescription(system: ArtSystem,
                                      components: MSZ[Bridge],
